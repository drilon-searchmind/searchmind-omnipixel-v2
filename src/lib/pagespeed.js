 ...
 * Fetches Core Web Vitals and performance metrics from Google PageSpeed Insights
 */

export async function fetchPageSpeedInsights(url) {
    try {
        // Note: In production, you should use your own API key
        // For now, we'll use a public API endpoint or create a server-side proxy
        const apiKey = process.env.PAGESPEED_API_KEY || process.env.NEXT_PUBLIC_PAGESPEED_API_KEY || '';
        
        // If no API key, return mock data structure
        if (!apiKey) {
            console.warn('No PageSpeed Insights API key found. Using mock data.');
            return {
                success: true,
                data: getMockPageSpeedData()
            };
        }
